...
import { FaListAlt, FaSave, FaTimes, FaEdit, FaTrashAlt, FaSearch } from 'react-icons/fa';
...
function CriarAtivSust() {
    const [atividade, setAtividade] = useState({id: 0, criar_nome: "", criar_cpf: "", criar_contato: "", criar_endereco: "", criar_bairro: "", criar_numero: "", tipoAtividade: {  id: 0, nome: "" }, criar_data: "", criar_horarioInicial: "", criar_horarioFinal: "", criar_descricao: "" });
        ...
        if (idAtivSust) {
            const obterAtivSust = async () => {
                try {
                    ...
                    setNome(dados.criar_nome);
                    setNome(dados.criar_nome);
                    setCpf(dados.criar_cpf);
                    setContato(dados.criar_contato);
                    setEndereco(dados.criar_endereco);
                    setBairro(dados.criar_bairro);
                    setNumero(dados.criar_numero);
                    setIdTipo(dados.criar_id);
                    setData(new Date(dados.criar_data).toISOString().slice(0, 10));
                    setHorarioInicial(dados.criar_horarioInicial);
                    setHorarioFinal(dados.criar_horarioFinal);
                    setDescricao(dados.criar_descricao);
                } catch (error) {
    ...
    const handleSalvar = async (event) => {
        ...
        try {
            ...
            navigate('/criarativsust'); // Navegar após a operação
    
        } catch (error) {
            console.error('Erro ao salvar a atividade:', error);
            if (error.response && error.response.data) {
                setErro(error.response.data.message); // Exibir mensagem de erro específica
            } else {
                setErro('Erro ao salvar a atividade. Verifique os dados.');
            }
        }
    };
    ...
    const handleEditar = (atividade) => {
        ...
        setAtividadeSelecionada(atividade); // Por exemplo
    };
    
    const handleExcluir = async (id) => {
        if (window.confirm('Tem certeza que deseja excluir?')) {
            try {
                await ativSustService.excluir(id); // Exclui a atividade pelo id
                setSucessoMensagem('Atividade excluída com sucesso!');
                await listarAtividades(); // Atualiza a lista
            } catch (error) {
                console.error('Erro ao excluir atividade:', error);
                setErro('Erro ao excluir a atividade. Tente novamente.');
            }
        }
    };
                                ...
                                {listaAtividades.map((atividade) => (
                                    <tr key={atividade.criar_id}>
                                        ...
                                        <td colSpan={2} className='text-center'>
                                            <Button variant="primary" className="btn-primary m-1" onClick={() => handleEditar(atividade)}>
                                                <FaEdit />
                                            </Button>
                                        </td>
                                        <td colSpan={2} className='text-center'>
                                        <td colSpan={2} className='text-center'>
                                            <Button variant="danger" className="text-danger text" onClick={() => handleExcluir(atividade.id)}>
                                                <FaTrashAlt />
                                            </Button>
                                        </td>
                                        </td>
